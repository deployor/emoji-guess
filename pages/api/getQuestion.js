...
export default async function handler(req, res) {
  ...
  if (security.detectBot(req)) {
    return res.status(403).json({ error: 'Suspicious request pattern' });
  }
  ...
  const lastRequest = session.gameState.lastRequestTime || 0;
  const timeSinceLastRequest = Date.now() - lastRequest;
  if (timeSinceLastRequest < 800) {
    const delay = Math.min(2000, (3 - Math.floor(timeSinceLastRequest/300)) * 1000);
    await new Promise(resolve => setTimeout(resolve, delay));
  }

  // Verify game time hasn't expired
  const gameTime = Date.now() - session.gameState.timeStarted;
  if (gameTime > parseInt(process.env.NEXT_PUBLIC_TIMER_DURATION) * 1000) {
    return res.status(403).json({ error: 'Game time expired' });
  }
  ...
  const questions = [
    ...
    // CREDIT TO OPENAI, i was infact too lazy to do this myself :)
  ...
  // Create a signed token for this question
  const questionToken = createHash('sha256')
    .update(`${session.id}:${randomIndex}:${question.answer}:${process.env.SECRET_KEY}`)
    .digest('hex');

  // Store current question in session
  session.gameState.currentQuestion = {
    id: randomIndex,
  ...
  const signature = security.sign(responseData, process.env.SECRET_KEY);

  session.gameState.lastRequestTime = Date.now();
  session.gameState.lastResponse = {
    signature,
